...
// ===== CLEVELAND CAVALIERS DATA & COMPONENTS =====
// Cavs 2025-26 remaining regular season games
const cavsRemainingGames: { date: string; opponent: string; time: string; home: boolean; note?: string }[] = [
  { date: "2026-04-08", opponent: "Atlanta Hawks", time: "7:00 PM", home: true, note: "Autism Awareness Night" },
  { date: "2026-04-10", opponent: "Atlanta Hawks", time: "7:00 PM", home: false },
  { date: "2026-04-12", opponent: "Washington Wizards", time: "6:00 PM", home: true, note: "Fan Appreciation Day" },
];

function getCavsGameStatus() {
  const now = new Date();
  const todayStr = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0');
  
  // Check if there's a game today
  const todayGame = cavsRemainingGames.find(g => g.date === todayStr);
  if (todayGame) {
    return { label: "TODAY'S GAME", game: todayGame, isToday: true };
  }
  
  // Find next upcoming game
  const upcoming = cavsRemainingGames.find(g => g.date > todayStr);
  if (upcoming) {
    const gameDate = new Date(upcoming.date + 'T12:00:00');
    const diffMs = gameDate.getTime() - now.getTime();
    const diffDays = Math.ceil(diffMs / (1000 * 60 * 60 * 24));
  ...
  // Regular season is over - show playoff status
  return null;
}

function CavsPlayoffCountdown() {
  // NBA Playoffs Round 1 typically starts around April 19
  const targetDate = new Date("2026-04-19T13:00:00-04:00").getTime();
  const [timeLeft, setTimeLeft] = useState({ days: 0, hours: 0, minutes: 0, seconds: 0 });
  ...
  return (
    <div className="text-center">
      <p className="font-heading text-sm text-white/60 uppercase tracking-[0.2em] mb-4">
        Countdown to 2026 NBA Playoffs
      </p>
      <div className="flex justify-center gap-3 sm:gap-5">
        {units.map((u, i) => (
          <div key={i} className="flex flex-col items-center">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-[#1a1a1a] border border-[#FFB81C]/40 rounded-lg flex items-center justify-center shadow-lg shadow-black/30">
              <span className="font-heading text-2xl sm:text-3xl text-white tabular-nums">
                {String(u.value).padStart(2, "0")}
              </span>
            </div>
      ...
      <p className="text-white/40 text-xs mt-4 italic">Playoff schedule subject to NBA confirmation.</p>
    </div>
  );
}

function CavsScheduleAccordion() {
  const [open, setOpen] = useState(false);

  return (
    <div className="border border-[#FFB81C]/20 rounded-lg overflow-hidden">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-5 py-4 bg-[#1a1a1a] hover:bg-[#222] transition-colors"
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-white/40 text-sm hidden sm:inline">({cavsRemainingGames.length} games)</span>
        </div>
        <ChevronDown
          className={`w-5 h-5 text-[#FFB81C] transition-transform duration-300 ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && (
        <div className="bg-[#111] px-3 sm:px-5 py-4">
          <div className="grid gap-1">
            {cavsRemainingGames.map((game, i) => {
              const d = new Date(game.date + 'T12:00:00');
              const dateLabel = d.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
              return (
                <div
                  key={i}
                  className="flex items-center justify-between py-2.5 px-3 rounded text-sm sm:text-base bg-white/5 border-l-2 border-[#FFB81C]/40 hover:bg-white/8 transition-colors"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                    <span className="font-heading text-[#FFB81C]/80 text-xs w-24 sm:w-28">{dateLabel}</span>
                    <span className="text-white/90">{game.home ? 'vs' : '@'} {game.opponent}</span>
                    {game.note && <span className="text-[#FFB81C]/50 text-xs italic">{game.note}</span>}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-white/50 font-mono text-xs sm:text-sm">{game.time}</span>
                    <span className={`text-xs font-heading uppercase tracking-wider px-2 py-0.5 rounded ${
          ...
          <div className="mt-4 pt-4 border-t border-white/10 text-center">
            <a
              href="https://www.nba.com/cavaliers/schedule"
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#FFB81C] hover:text-[#ffc94d] text-sm font-heading uppercase tracking-wider transition-colors"
            >
              Full Schedule on NBA.com &rarr;
            </a>
          </div>
        </div>
      )}
...
function CavaliersSection() {
  return (
    <section className="relative overflow-hidden">
      {/* Background: Cavs arena vibe with wine-dark overlay */}
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{ backgroundImage: `url(${IMAGES.cavs})` }}
      />
      <div className="absolute inset-0 bg-gradient-to-b from-[#2a0f1a]/95 via-[#1a0a12]/92 to-[#1a1a1a]" />

      {/* Wine/gold accent line at top */}
      <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#FFB81C] to-transparent" />

      <div className="relative z-10 max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        {/* Header */}
        <div className="text-center mb-10 sm:mb-14">
          {/* Cavaliers sword logo circle */}
          <div className="w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-6 rounded-full bg-[#6F263D] flex items-center justify-center shadow-2xl shadow-[#6F263D]/40 border-2 border-[#FFB81C]/30">
            <span className="font-heading text-3xl sm:text-4xl text-[#FFB81C] font-bold">CLE</span>
          </div>
          <h2 className="font-heading text-3xl sm:text-4xl lg:text-5xl text-white uppercase tracking-tight mb-4">
            Cleveland Cavaliers
          </h2>
          <div className="flex items-center justify-center gap-3 mb-4">
            <span className="font-heading text-lg sm:text-xl text-[#FFB81C] uppercase tracking-wider">50-29</span>
            <span className="text-white/30">|</span>
            <span className="font-heading text-lg sm:text-xl text-[#FFB81C] uppercase tracking-wider">4th Seed East</span>
            <span className="text-white/30">|</span>
            <span className="inline-flex items-center gap-1.5 bg-[#FFB81C]/15 border border-[#FFB81C]/30 px-3 py-1 rounded-full">
              <span className="w-2 h-2 rounded-full bg-[#FFB81C] animate-pulse" />
              <span className="font-heading text-xs text-[#FFB81C] uppercase tracking-wider">Playoffs Clinched</span>
            </span>
          </div>
          <p className="text-white/70 text-lg sm:text-xl max-w-2xl mx-auto leading-relaxed">
            The Cavs are locked in for the postseason. Watch every game at Spunkmeyers with the best crowd in Wadsworth.
          </p>
        </div>

        {/* Three player photos row */}
        <div className="flex justify-center gap-3 sm:gap-6 mb-10 sm:mb-14">
          <div className="w-28 h-40 sm:w-44 sm:h-60 rounded-lg overflow-hidden border-2 border-[#FFB81C]/25 shadow-xl shadow-black/40 -rotate-3 hover:rotate-0 transition-transform duration-500">
            <img
              src={IMAGES.cavsHarden}
              alt="James Harden - Cleveland Cavaliers"
              className="w-full h-full object-cover object-top"
            />
          </div>
          <div className="w-28 h-40 sm:w-44 sm:h-60 rounded-lg overflow-hidden border-2 border-[#FFB81C]/25 shadow-xl shadow-black/40 hover:scale-105 transition-transform duration-500 -mt-2">
            <img
              src={IMAGES.cavsMitchell}
              alt="Donovan Mitchell - Cleveland Cavaliers"
              className="w-full h-full object-cover object-top"
            />
          </div>
          <div className="w-28 h-40 sm:w-44 sm:h-60 rounded-lg overflow-hidden border-2 border-[#FFB81C]/25 shadow-xl shadow-black/40 rotate-2 hover:rotate-0 transition-transform duration-500 mt-3">
            <img
              src={IMAGES.cavsMobley}
              alt="Evan Mobley - Cleveland Cavaliers"
              className="w-full h-full object-cover object-top"
            />
          </div>
        </div>
        ...
        <div className="max-w-xl mx-auto mb-10 sm:mb-14">
          {(() => {
            const status = getCavsGameStatus();
            if (!status) {
              return (
                <div className="bg-[#6F263D]/15 border border-[#FFB81C]/20 rounded-lg px-6 py-5 text-center">
                  <p className="font-heading text-sm text-[#FFB81C] uppercase tracking-[0.2em] mb-2">2025-26 Season</p>
                  <p className="font-heading text-2xl sm:text-3xl text-white uppercase">Regular Season Complete</p>
                  <p className="text-white/70 text-lg mt-2">Playoffs start soon. Stay locked in at Spunkmeyers.</p>
                </div>
              );
            }
            const gameDate = new Date(status.game.date + 'T12:00:00');
            ...
            return (
              <div className={`rounded-lg overflow-hidden border ${
                status.isToday 
                  ? 'border-[#FFB81C] shadow-lg shadow-[#6F263D]/30' 
                  : 'border-[#FFB81C]/20'
              }`}>
                {/* Status header bar */}
                <div className={`px-5 py-2.5 flex items-center justify-center gap-2 ${
                  ...
                  {status.isToday && (
                    <span className="w-2.5 h-2.5 rounded-full bg-[#FFB81C] animate-pulse" />
                  )}
                  <span className="font-heading text-sm text-white uppercase tracking-[0.2em]">
                    {status.label}
                  </span>
                ...
                {/* Game details */}
                <div className="bg-[#1a0a12]/80 px-5 py-5 text-center">
                  <p className="text-white/50 text-sm mb-1">{dateFormatted}</p>
                  <p className="font-heading text-2xl sm:text-3xl text-white uppercase leading-tight">
                    {status.game.home ? 'vs ' : '@ '}{status.game.opponent}
                  </p>
                  <div className="flex items-center justify-center gap-4 mt-3">
                    <span className="font-heading text-lg text-[#FFB81C]">{status.game.time} ET</span>
                    <span className={`text-xs font-heading uppercase tracking-wider px-2 py-0.5 rounded ${
                      status.game.home 
                        ? 'bg-[#FFB81C]/15 text-[#FFB81C]' 
                        : 'bg-white/10 text-white/60'
                  ...
                  {status.game.note && (
                    <p className="text-[#FFB81C]/60 text-sm mt-2 italic">{status.game.note}</p>
                  )}
                </div>
              </div>
            );
        ...
        <div className="max-w-xl mx-auto mb-10 sm:mb-14">
          <div className="space-y-3">
            <div className="flex items-center gap-3 justify-center">
              ...
              <span className="text-white/80 text-base">Every Cavs game on our big screens</span>
            </div>
            <div className="flex items-center gap-3 justify-center">
              <div className="w-3 h-3 rounded-full bg-[#FFB81C]" />
              <span className="text-white/80 text-base">Playoff watch parties all postseason</span>
            </div>
            <div className="flex items-center gap-3 justify-center">
              <div className="w-3 h-3 rounded-full bg-[#FFB81C]" />
              <span className="text-white/80 text-base">The loudest bar in Wadsworth when the Cavs are on</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-10 sm:mb-14">
          <CavsPlayoffCountdown />
        </div>

        {/* Schedule Accordion */}
        <div className="max-w-3xl mx-auto">
          <CavsScheduleAccordion />
        </div>
      </div>
    </section>
  );
